...
pub trait SystemFunc {
    fn call(&mut self, world: &mut World, api: &mut dyn DoryenApi);
}
...
impl SystemFunc for InputSystem {
    fn call(&mut self, world: &mut World, api: &mut dyn DoryenApi) {
        let player_pos_query = world.query_mut::<(&mut Position, &mut Player)>();

        let input = api.input();

        for (_id, (player_pos, _player)) in player_pos_query {
